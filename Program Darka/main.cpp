...
//	else {
		if(!czyGral(pom[0],pom[1]) && !czyGral(pom[2],pom[3]) && !czyGral(pom[4],pom[5])){
	        ...
	        numerStolu++;
	        paring[0][numerStolu] = pom[4].getId();
	        paring[1][numerStolu] = pom[5].getId();
	        int x = (*pom->getId();
			...
			pomocnicza = szukajGracza(table ,pom[4].getNick(), ilosc_old);
			if(pomocnicza>=0) (table+pomocnicza)->addPrzeciwnik(pom[5].getNick(), whichBattle);	
			pomocnicza = szukajGracza(table ,pom[5].getNick(), ilosc_old);
			if(pomocnicza>=0) (table+pomocnicza)->addPrzeciwnik(pom[4].getNick(), whichBattle);				
		}
