...
bool Paring(CGracz *table, int ilosc, int whichBattle);
...
int main()
{
    ...
    while(1){
        //PARING
        cout<<"paruje"<<endl;
        Paring(table, ilosc_graczy, numBitwy);
        //WYPISYWANIE GRACZY
        cout<<"wypisuje"<<endl;
        Wypis_Graczy(table, ilosc_graczy);
        //DODAWANIE PUNKTOW
        for(int i=0;i<ilosc_graczy;i++){
            ...
            string nickToAdd;
            cout<<"Podaj numer gracza, VP, GP oraz numer bitwy: ";
//            cin>> VPtoAdd;
...
bool Paring(CGracz *table, int ilosc, int whichBattle ){
    ...
    string paring[2][ilosc/2];
    string paring_zapas[2][2];    
    string pauzaName = " ";
    CGracz pom[ilosc];
        ...
        while(!pauza && i>=0){
          if((table + i)->getPauza() == false){
           // cout<<"znaleziono pauzujacego"<<endl;
            ...
            cout<<"pobranie nazwy pauzujacego"<<endl;
...
//	else {
		if(!czyGral(pom[0],pom[1]) && !czyGral(pom[2],pom[3])){
			paring[0][numerStolu] = pom[0].getNick();
	        paring[1][numerStolu] = pom[1].getNick();
	        numerStolu++;
	        paring[0][numerStolu] = pom[2].getNick();
	        paring[1][numerStolu] = pom[3].getNick();
			...
			paring[0][numerStolu] = pom[0].getNick();
	        paring[1][numerStolu] = pom[2].getNick();
	        numerStolu++;
	        paring[0][numerStolu] = pom[1].getNick();
	        paring[1][numerStolu] = pom[3].getNick();
...
//	}
    ...
    for(int x = 0; x<(ilosc_old/2);x++){
        cout<<numerStolu<< " "<< paring[0][x]<< " "<< paring[1][x]<<endl;
        numerStolu++;
    }
    pomocnicza = szukajGracza(table ,"1", ilosc_old);
    for(int x = 0; x<=4 ; x++){
